...
namespace Team75
{
    public partial class approveAnnualAccidental : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Session["LastRequestId_AnAcc"] == null)
                {
                    lblInfo.Text = "No annual/accidental leave request was processed in this session.";
                    LoadEmpty();
                    return;
                }

                int requestId = Convert.ToInt32(Session["LastRequestId_AnAcc"]);
                int? hrId = null;
