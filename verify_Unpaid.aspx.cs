...
namespace Team75
{
    public partial class verify_Unpaid : System.Web.UI.Page
    {
        ...
        protected void btnProceed_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(ConnectionString))
                {
                    ...
                    // Check that this request exists in Leave
                    ...
                    // Check that this request is really an Unpaid_Leave
                    using (SqlCommand cmdCheckType = new SqlCommand(
                        "SELECT COUNT(*) FROM Unpaid_Leave WHERE request_ID = @req", conn))
                    {
                        ...
                        {
                            lblError.Text =
                                "Cannot approve or reject this request because it is not an unpaid leave.";
                            lblError.ForeColor = Color.Red;
                            return;
                        }
                    }

                    // Call stored procedure HR_approval_Unpaid
                    ...
                    Session["LastRequestId_Unpaid"] = requestID;
                    ...
                    string script =
                        "alert('The Unpaid Leave has been " + safeStatus +
                        " Successfully'); window.location='ApproveUnpaid.aspx';";
            ...
            {
                lblError.Text = "Error while approving/rejecting unpaid leave: " + ex.Message;
                lblError.ForeColor = Color.Red;
            }
        }

        // Helper: Load Leave + Unpaid_Leave rows
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(ConnectionString))
                {
                    ...
                    SqlDataAdapter da2 = new SqlDataAdapter(
                        "SELECT * FROM Unpaid_Leave WHERE request_ID = @r", conn);
                    da2.SelectCommand.Parameters.AddWithValue("@r", requestID);
                    DataTable dt2 = new DataTable();
                    da2.Fill(dt2);
                    gridUnpaid.DataSource = dt2;
                    gridUnpaid.DataBind();
                    gridUnpaid.Visible = true; // show even if empty so HR sees it's not unpaid
